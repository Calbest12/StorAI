...
app.use(cors({
  ...
  function extractRelevantPassages(text, query, numSentences = 3) {
    const stopWords = [
    ...
    const queryWords = query.toLowerCase().split(/\s+/).filter(word => !stopWords.includes(word));


    // Calculate scores for each sentence based on query word frequency
    let scoredSentences = sentences.map(sentence => {
      const words = sentence.toLowerCase().split(/\s+/);
      const score = queryWords.reduce((acc, word) => {
        const termFrequency = words.filter(w => w === word).length;
        return acc + termFrequency;
      }, 0);
    ...
    let result = "";
    let maxChars = 450;
    for (let {sentence} of scoredSentences) {
        if (result.length + sentence.length <= maxChars) {
            ...
            if (spaceLeft > 0) {
                ...
                if (lastSpaceIndex > -1) {
                    snippet = snippet.substring(0, lastSpaceIndex).trim() + "...";
                }
                result += snippet;
            }
            break;
    ...
    if (results.length > 0) {
      results.forEach(result => {
        if (result.pageContent) {
          const relevantPassages = extractRelevantPassages(result.pageContent, query, 7); // Requesting 3 sentences
